...
let Suggestionscount = 1 // Quite important variable for the suggestion system
...
function Eventsubmitsc() {
    ...
    if (Eventinput.value.length == 0 && Eventinput2.value.length == 0) { // Checks if both input fields are empty if yes then error message for the user//
    ...
    else if (Eventinput2.value == "neger") { // Checks if Input are any bad words (N Bomb)//
        Eventinput2.value = "#####"
        alert("So nicht min jung")
        document.getElementById("Eventsubmit").style.display = "none"


    } else if (Eventinput.value == "neger") {
